...
ALTER TABLE `showtimes` MODIFY COLUMN `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP;--> statement-breakpoint
...
ALTER TABLE `showtimes` ADD `room_id` varchar(36) NOT NULL;--> statement-breakpoint
ALTER TABLE `showtimes` ADD `starts_at` datetime NOT NULL;--> statement-breakpoint
ALTER TABLE `showtimes` ADD `updated_at` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP NOT NULL;--> statement-breakpoint
...
ALTER TABLE `showtimes` ADD CONSTRAINT `uq_showtime_room_time` UNIQUE(`room_id`,`starts_at`);--> statement-breakpoint
...
ALTER TABLE `tickets` ADD CONSTRAINT `tickets_showtime_id_showtimes_id_fk` FOREIGN KEY (`showtime_id`) REFERENCES `showtimes`(`id`) ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE `booking_seats` ADD CONSTRAINT `booking_seats_showtime_id_showtimes_id_fk` FOREIGN KEY (`showtime_id`) REFERENCES `showtimes`(`id`) ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE `bookings` ADD CONSTRAINT `bookings_showtime_id_showtimes_id_fk` FOREIGN KEY (`showtime_id`) REFERENCES `showtimes`(`id`) ON DELETE restrict ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE `showtimes` ADD CONSTRAINT `showtimes_movie_id_movies_id_fk` FOREIGN KEY (`movie_id`) REFERENCES `movies`(`id`) ON DELETE restrict ON UPDATE no action;--> statement-breakpoint
ALTER TABLE `showtimes` ADD CONSTRAINT `showtimes_cinema_id_cinemas_id_fk` FOREIGN KEY (`cinema_id`) REFERENCES `cinemas`(`id`) ON DELETE restrict ON UPDATE no action;--> statement-breakpoint
ALTER TABLE `showtimes` ADD CONSTRAINT `showtimes_room_id_rooms_id_fk` FOREIGN KEY (`room_id`) REFERENCES `rooms`(`id`) ON DELETE restrict ON UPDATE no action;--> statement-breakpoint
...
CREATE INDEX `idx_showtime_movie_time` ON `showtimes` (`movie_id`,`starts_at`);--> statement-breakpoint
CREATE INDEX `idx_showtime_cinema_time` ON `showtimes` (`cinema_id`,`starts_at`);--> statement-breakpoint
...
ALTER TABLE `showtimes` DROP COLUMN `show_date`;--> statement-breakpoint
ALTER TABLE `showtimes` DROP COLUMN `show_time`;